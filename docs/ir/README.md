...
### 앱 화면 캡처 — 파일만 떨궈두면 자동으로 들어갑니다

`docs/ir/`에 아래 이름으로 넣고 `node build_deck.js`를 다시 돌리면 점선 자리가 채워진다.
없으면 점선 자리로 남는다. 세로 화면(9:16) 기준.

| 파일 | 화면 | 들어가는 곳 |
|---|---|---|
| `shot-scan.png` | 카메라 QR·NFC 스캔 | 22 USER FLOW |
| `shot-register.png` | 등록 완료 · 타임라인 첫 줄 | 22 |
| `shot-record.png` | 기록 작성 (상황 칩) | 22 |
| `shot-remade.png` | REMADE 시안 선택 | 22 |
